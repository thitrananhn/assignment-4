...
/* 15) A query to find thetotalcost (excluding fees and taxes) of all equipment in each invoice
*/
...
/* 16) A query to detect invalid datawhich finds any invoice that includes multipleinstances of the same equipment product.
*/ 
...
SELECT ip.invoiceKey, ip.productKey, COUNT(*) duplicateAmount FROM InvoiceProductList ip
	JOIN Invoice i ON ip.invoiceKey = i.invoiceKey
	GROUP BY invoiceKey, productKey
    HAVING COUNT(*) > 1;
/* 17) A query to find any and all invoices where thesalesperson is the same as the primary contact of the invoice’s customer.
*/
